...
BSTs are useful when it comes to queries such as searching for a key in a tree that provides lookup of(h) where h is the height of the tree, and it is proved that h = log(N), faster than going through the array of elements in O(N). Cutting the search cost from O(N) to O(lgN) is significant to applications that can only afford a logarithmic operation in that structure. This table is a small comparison of three basic operations in <emp>average case</emp> using Big O notation.
<code>Number of nodes: n = 1 + 2 + 4 + 8 + ... + 2^h-1 = 2^h - 1</code>
<code>Height of the tree: h = log(n+1)</code>

<table cellpadding="0" cellspacing="0">
	<tr>
		<th>Operation</th><th>Search()</th><th>Delete()</th><th>Insert()</th>
	</tr>
	<tr>
		<td>Array</td><td>O(N)</td><td>O(N)</td><td>O(N)</td>
	</tr>
	<tr>
		<td>Binary Search Tree</td><td>O(lgN)</td><td>O(lgN)</td><td>O(lgN)</td>
	</tr>
</table>
</div>
...
<h3> 3. Node (N) to delete has two children </h3>
